        ...
        {specials.length > 0 ? (
          <div className="mt-2 rounded-lg bg-ivory px-3 py-2">
            ...
            <ul className="mt-1 space-y-0.5">
              {specials.map((s) => (
                <li key={s.label} className="text-[12px] leading-snug text-text-soft">
                  {s.label}
...
export default function GolfPriceBoard({ courses }: { courses: GolfCourse[] }) {
  const groups = REGION_ORDER.map((region) => ({
    region,
    items: courses.filter((c) => c.location.includes(region)),
  })).filter((g) => g.items.length > 0);
  const grouped = new Set(groups.flatMap((g) => g.items.map((c) => c.slug)));
  const rest = courses.filter((c) => !grouped.has(c.slug));
  if (rest.length > 0) groups.push({ region: "기타", items: rest });
  ...
  return (
    <div className="rounded-2xl border border-border bg-ivory p-4 md:p-6">
      ...
      <div className="mt-6 space-y-7">
        {groups.map((g) => (
          <div key={g.region}>
            <p className="mb-3 inline-block rounded-full bg-forest px-3.5 py-1 text-[13px] font-medium text-white">
              {g.region}
            </p>
